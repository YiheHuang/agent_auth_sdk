...
`local` 模式仍使用真实 Registry、Vault 和持久化 nonce，但只允许 loopback，并允许本机 HTTP。它用于集成验证，不等同 production TLS 边界。
...
- 远程 endpoint 连接固定到已校验的公网 IP，并保留原始 Host 与 TLS SNI，避免 DNS rebinding。
